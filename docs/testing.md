...
- `ContactChannelWorkflowTests`: that guidance reaches the specialist context, that an unmet preference is recorded as `workflow.contact_channel_unavailable`, and — in **both** directions — that neither `requires_approval` nor the invoked tools differ between a workflow carrying an unserviceable preference and one carrying none.
  - The two directions need different requests, which is the point worth remembering. Comparing two informational runs cannot detect de-escalation: approval is false either way, so the assertion passes by saying false equals false. The de-escalation case therefore runs a **dispute**, where approval is genuinely true and a regression has something to break. Both cases were confirmed by mutation — forcing approval on and forcing it off each fails exactly one of them.
