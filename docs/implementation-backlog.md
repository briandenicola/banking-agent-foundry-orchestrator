...
16. P2 — Propagate the signed-in user with an on-behalf-of flow
    - Today the Web UI reads the platform-verified `X-MS-CLIENT-PRINCIPAL-ID` header and passes that object ID to the orchestrator as a *value*, which the orchestrator asserts as the memory scope while calling Foundry as its own managed identity. It is identity propagation by assertion, not by token. The orchestrator trusts the caller to state who the customer is, which is only sound because the orchestrator has internal ingress and the Web UI is the sole caller.
    - **This is not needed for per-customer memory.** Inline scoping already isolates memory per customer, verified by [`verify-memory-scope.py`](../scripts/verify-memory-scope.py) and by the absence of the `EnforceScope` mismatch warning in a live run. The case for OBO is the trust model: an exchanged token would let the orchestrator verify the customer rather than take the Web UI's word for it, and would make the identity chain demonstrable rather than asserted.
    ...
    - Architectural limit worth recording before anyone attempts it: workflows resume in the background through the recovery worker, long after a user token has expired. OBO can only cover the interactive path. Persisting refresh tokens to bridge that gap was rejected — see the header of [`apps/webui-auth.tf`](../apps/webui-auth.tf) — because it puts user credentials at rest in the workflow store.
    - Requires a confidential-client credential for the Web UI, which bumps against the no-keys-for-service-authentication rule and so needs an ADR. Evaluate a federated identity credential first; cross-tenant federation with an Azure managed identity as the issuer is unverified here.
    - Acceptance criteria: the orchestrator derives the customer from a validated token rather than a request field, a request asserting a different customer than the token carries is rejected, and the background workflow path continues to work without any user token.
