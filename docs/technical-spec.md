...
- MCP and typed agent integration
  ...
  - Uses real MCP only for `transaction.explain`; other tools use the typed envelope until migrated.
...
5. The current orchestrator invokes the planner through the typed Foundry hosted-agent HTTP envelope. The transaction-explanation specialist uses MCP JSON-RPC when `FOUNDRY_MCP_TOOL_ENDPOINTS` contains `transaction.explain`; the other specialists still use the typed envelope.
