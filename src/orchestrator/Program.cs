...
// Delegated user authentication for the interactive path.
//
// Distinct from service authentication above, and deliberately so. Service auth
// proves that *the Web UI* is calling; this proves *which customer* it is
// calling for. They also fail for different reasons: service auth needs an
// api:// identifier URI in the deployment's own tenant, which some tenants deny
// (issue #30), whereas this uses the Web UI's own client credential and so can
// be pointed at a tenant the operator controls.
//
...
var delegationTenantId = builder.Configuration["USER_DELEGATION_TENANT_ID"];
var apiAppId = builder.Configuration["ORCHESTRATOR_API_APP_ID"];

if (userDelegationEnabled && serviceAuthEnabled)
{
    throw new InvalidOperationException(
        "USER_DELEGATION_ENABLED and SERVICE_AUTH_ENABLED cannot both be true. They register different "
...
{
    ArgumentException.ThrowIfNullOrWhiteSpace(delegationTenantId, "USER_DELEGATION_TENANT_ID");
    ArgumentException.ThrowIfNullOrWhiteSpace(apiAppId, "ORCHESTRATOR_API_APP_ID");

    var delegationIssuer = $"https://login.microsoftonline.com/{delegationTenantId}/v2.0";
    var apiAudience = $"api://{apiAppId}";

    builder.Services
        .AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
        .AddJwtBearer(options =>
        {
            options.Authority = delegationIssuer;
...
builder.Services.AddSingleton<ICustomerAssertionGuard>(sp => userDelegationEnabled
