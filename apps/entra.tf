locals {
  ...
  # Token scope the Web UI requests; audience the orchestrator validates.
  orchestrator_token_scope = "api://${azuread_application.orchestrator_api.client_id}/.default"
}
...
resource "azuread_application_identifier_uri" "orchestrator_api" {
  application_id = azuread_application.orchestrator_api.id
  identifier_uri = "api://${azuread_application.orchestrator_api.client_id}"
}
...
resource "azuread_service_principal" "orchestrator_api" {
  client_id = azuread_application.orchestrator_api.client_id
}
...
resource "azuread_app_role_assignment" "webui_workflow_invoke" {
  ...
  resource_object_id  = azuread_service_principal.orchestrator_api.object_id
}
